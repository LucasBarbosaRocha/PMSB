...
public:
    ...
    void changeGraph(unordered_map<int, int> maping, string sequence);
    ...
    vector<double> getCosts(string kmer);
...
void Hash::enrichGraph()
{
    ...
    for (itr = this->new_graph.begin(); itr != this->new_graph.end(); itr++)
    {
        itr->second.node_in_sequence_graph = i++;
    }   
}

void Hash::changeGraph(unordered_map<int, int> maping, string sequence)
{
    unordered_map<string, My_object>:: iterator itr;
    list<string>::iterator it;
    int pos;
    vector<string> deletar, inserir;
    for (itr = this->new_graph.begin(); itr != this->new_graph.end(); itr++)
    {
        pos = itr->second.node_in_sequence_graph;
        if (maping.find(pos) != maping.end())
        {
            deletar.push_back(itr->first);
            inserir.push_back(sequence.substr(maping[pos],this->k));
        }
    }  

    for (auto del : deletar) 
    {
        this->new_graph.erase(del);
    }

    for (auto ins : inserir)
    {
        this->insertKmer(ins);
    }
...
vector<double> Hash::getCosts(string kmer)
{
    vector<double> costs; int cost;
    unordered_map<string, My_object>:: iterator itr;
    list<string>::iterator it;
    for (itr = this->new_graph.begin(); itr != this->new_graph.end(); itr++)
    {
        cost = this->hammingDistance(kmer, itr->first);
...
vector<int> Hash::findAnchors(string sequence)
{
    ...
    {   
        ...
        if (this->contains(kmer_sequence))
