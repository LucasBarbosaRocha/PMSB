...
public:
    ...
    int heuristic(SequenceGraph grafo, string u, string q, int i, int node_start);

    int h(SequenceGraph grafo, string u, string s, int node_start);

    int recursiveAlign(SequenceGraph grafo, string u, string s, int node_start, int curr, int best);
...
{
    int d = min(q.size(),  q.size() - i);
    string s = q.substr(i,i+d);
    return h(grafo, u, s, node_start);

}

int Marschall::h(SequenceGraph grafo, string u, string s, int node_start)
{
    return recursiveAlign(grafo, u, s, node_start, 0, 1000);
}


int Marschall::recursiveAlign(SequenceGraph grafo, string u, string s, int node_start, int curr, int best)
{
    if (curr > best)
        return best;
    if (s.size() == 1)
        return curr;
    for (auto i = grafo.getAdjBegin(node_start); i != grafo.getAdjEnd(node_start); i++)
    {
        string suf = s.substr(1);
        best = recursiveAlign(grafo, u, suf, i->first, curr + i->second, best);
    }
    return best;
}

// Dijkstra
pair<vector<pair<int,string>>, int> Marschall::aStar(SequenceGraph grafo, int orig, int dest, string uM, string qM)
{
    // vetor de distâncias
    int V = grafo.getV();
    list<string> induced_sequence;
        ...
        if(visitados[u] == false)
        {
			...
			if (grafo.getOutDegree(u) > 0)
			{
                ...
                for(auto it = grafo.getAdjBegin(u); it !=  grafo.getAdjEnd(u); it++)
                {
                    ...
                    int f = dist[u] + custo_aresta + h(grafo, uM, qM, v);
