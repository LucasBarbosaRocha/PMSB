...
public:
	...
	pair<list<pair<int,string>>, list<pair<int,int>>> bfs(int v, int limite);
...
pair<list<pair<int,string>>, list<pair<int,int>>> SequenceGraph::bfs(int v, int limite)
{
	...
	while(true)
	{
		for(auto it = this->getAdjBegin(v); it != this->getAdjEnd(v); it++)
		{
			if(!visitados[(*it).first])
			{
				...
				//cout << "Visitando vertice " << (*it).first << " ... level " <<  this->level[(*it).first] << "\n";
                if (this->level[(*it).first] < limite)
                {
                    //cout << "O vertice " << (*it).first << " vai entrar" << endl;
	...
	return make_pair(vertices,arestas);
}
